  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-6">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-4">
        <div className="glass-surface rounded-3xl p-5"><p className="micro-label neon-subtle">Tenants</p><p className="mt-2 text-2xl command-title text-slate-100">{summary?.totalTenants ?? 0}</p></div>
        <div className="glass-surface rounded-3xl p-5"><p className="micro-label neon-subtle">Active</p><p className="mt-2 text-2xl command-title text-slate-100">{summary?.activeTenants ?? 0}</p></div>
        <div className="glass-surface rounded-3xl p-5"><p className="micro-label neon-subtle">Pending Approval</p><p className="mt-2 text-2xl command-title text-slate-100">{summary?.pendingApproval ?? 0}</p></div>
        <div className="glass-surface rounded-3xl p-5"><p className="micro-label neon-subtle">Pending Payments</p><p className="mt-2 text-2xl command-title text-slate-100">{summary?.pendingPayments ?? 0}</p></div>
        <div className="glass-surface rounded-3xl p-5"><p className="micro-label neon-subtle">Active Codes</p><p className="mt-2 text-2xl command-title text-slate-100">{summary?.activeDiscountCodes ?? 0}</p></div>
      </section>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1.2fr_0.8fr] gap-6">
        <section className="glass-surface rounded-3xl p-6 border border-white/10 space-y-4">
          <div className="flex items-center justify-between gap-3">
            ...
            <p className="text-sm text-slate-400">Approve after payment, or approve early and wait for payment to activate.</p>
          </div>
        ...
        <div className="space-y-6">
          ...
          <section className="glass-surface rounded-3xl p-6 border border-white/10">
            ...
            <form onSubmit={handleSaveDiscount} className="space-y-3">
              <input name="code" value={discountForm.code} onChange={handleDiscountChange} required placeholder="CODE" className="w-full rounded-2xl border border-white/10 bg-slate-900/30 px-4 py-3" />
              <input name="name" value={discountForm.name} onChange={handleDiscountChange} placeholder="Display name" className="w-full rounded-2xl border border-white/10 bg-slate-900/30 px-4 py-3" />
              <textarea name="description" value={discountForm.description} onChange={handleDiscountChange} placeholder="Discount description" className="w-full rounded-2xl border border-white/10 bg-slate-900/30 px-4 py-3 min-h-[96px]" />
              <div className="grid grid-cols-2 gap-3">
                <input name="setupFeeNaira" type="number" value={discountForm.setupFeeNaira} onChange={handleDiscountChange} placeholder="Setup fee (NGN)" className="w-full rounded-2xl border border-white/10 bg-slate-900/30 px-4 py-3" />
                <input name="studentFeeNaira" type="number" value={discountForm.studentFeeNaira} onChange={handleDiscountChange} placeholder="Student fee / term (NGN)" className="w-full rounded-2xl border border-white/10 bg-slate-900/30 px-4 py-3" />
              </div>
              <input name="planScope" value={discountForm.planScope} onChange={handleDiscountChange} placeholder="growth,custom" className="w-full rounded-2xl border border-white/10 bg-slate-900/30 px-4 py-3" />
              <input name="endsAt" type="datetime-local" value={discountForm.endsAt} onChange={handleDiscountChange} className="w-full rounded-2xl border border-white/10 bg-slate-900/30 px-4 py-3" />
            ...
            <div className="mt-5 space-y-3">
              {discountCodes.map(discountCode => (
                <div key={discountCode.code} className="rounded-2xl bg-slate-900/30 p-4 text-sm text-slate-300">
                  <div className="flex items-center justify-between gap-3">
                    <p className="text-slate-100 font-semibold">{discountCode.code}</p>
                    <span className={`micro-label ${discountCode.active ? 'accent-emerald' : 'accent-slate'}`}>{discountCode.active ? 'active' : 'ended'}</span>
                  </div>
                  <p className="mt-2">{discountCode.description}</p>
                  <p className="mt-2">Setup: {discountCode.setupFee ? currencyFormatter.format(discountCode.setupFee) : 'unchanged'}</p>
                  <p className="mt-1">Student: {discountCode.studentFeePerTerm ? currencyFormatter.format(discountCode.studentFeePerTerm) : 'unchanged'}</p>
                  {discountCode.active && (
                    <button
                      type="button"
                      onClick={() => runAction(`end-${discountCode.code}`, async () => {
                        await endDiscountCode(discountCode.code);
                        setNotice(`${discountCode.code} ended.`);
                      })}
                      disabled={busyAction === `end-${discountCode.code}`}
                      className="mt-3 rounded-2xl border border-rose-400/40 px-3 py-2 font-semibold text-rose-200 disabled:opacity-40"
                    >
                      End Code
                    </button>
                  )}
          ...
          <section className="glass-surface rounded-3xl p-6 border border-white/10">
            ...
            <div className="space-y-3 text-sm text-slate-300 max-h-[420px] overflow-y-auto pr-1">
              ...
              )) : <p className="text-slate-400">No onboarding payments yet.</p>}
            </div>
          </section>
        </div>
      </div>
