...
function renderSchoolHome(tenant: any, branding: any, sections: any[], events: any[], options: { platformLoginUrl?: string } = {}) {
  ...
  ${loginScript()}
  <script>(function(){var rail=document.querySelector('.hero-media-rail');if(!rail)return;var card=rail.querySelector('.hero-media-card');if(!card)return;function step(){var w=card.getBoundingClientRect().width+14;if(rail.scrollLeft+rail.clientWidth>=rail.scrollWidth-8){rail.scrollTo({left:0,behavior:'smooth'});}else{rail.scrollBy({left:w,behavior:'smooth'});}}var timer=setInterval(step,3500);rail.addEventListener('mouseenter',function(){clearInterval(timer);});rail.addEventListener('mouseleave',function(){timer=setInterval(step,3500);});})();</script>`
