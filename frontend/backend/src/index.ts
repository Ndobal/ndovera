...
function renderSchoolHome(tenant: any, branding: any, sections: any[], events: any[], options: { platformLoginUrl?: string } = {}) {
  ...
  ${loginScript()}
  <script>(function(){var s=document.querySelectorAll('.hero-bg-slide');if(s.length<2)return;var i=0;setInterval(function(){s[i].classList.remove('is-active');i=(i+1)%s.length;s[i].classList.add('is-active');},4500);})();</script>`
