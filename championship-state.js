...
function resolveRuntimePath(envValue, fallbackPath) {
...
function parseJsonObject(value) { if (!value || typeof value !== 'string')
    return null; try {
...
function parseJsonArray(value) { if (!value || typeof value !== 'string')
    return []; try {
    const parsed = JSON.parse(value);
...
function rowToFlag(row) { return { name: String(row.name), enabled: Boolean(row.enabled) }; }
function rowToCompetition(row) {
    return {
        ...
        type: String(row.type), scope: String(row.scope), mode: String(row.mode), entryFee: Number(row.entry_fee || 0),
        status: String(row.status), startTime: row.start_time ? String(row.start_time) : null, endTime: row.end_time ? String(row.end_time) : null,
        ...
        joined: row.joined != null ? Boolean(row.joined) : undefined, participantStatus: row.participant_status ? String(row.participant_status) : null,
...
function rowToParticipant(row) {
    return { id: String(row.id), competitionId: String(row.competition_id), userId: String(row.user_id), schoolId: row.school_id ? String(row.school_id) : null, score: Number(row.score || 0), status: String(row.status), joinedAt: String(row.joined_at), submittedAt: row.submitted_at ? String(row.submitted_at) : null, totalTimeTaken: Number(row.total_time_taken || 0), violationCount: Number(row.violation_count || 0) };
}
...
async function readFeatureFlagsRaw() {
    if (pgPool)
        return (await pgPool.query('SELECT name, enabled FROM championship_feature_flags ORDER BY name ASC')).rows.map(rowToFlag);
    return sqlite ? sqlite.prepare('SELECT name, enabled FROM championship_feature_flags ORDER BY name ASC').all().map(rowToFlag) : [];
}
async function writeFeatureFlagRaw(name, enabled) {
    if (pgPool) {
...
async function ensureSchema() {
    ...
    schemaPromise = (async () => {
        if (pgPool) {
			...
			`);
            const competitionColumns = sqlite.prepare('PRAGMA table_info(championship_competitions)').all().map((column) => column.name);
            if (!competitionColumns.includes('host_organization'))
                sqlite.exec('ALTER TABLE championship_competitions ADD COLUMN host_organization TEXT');
            if (!competitionColumns.includes('hosted_by_ndovera'))
                sqlite.exec('ALTER TABLE championship_competitions ADD COLUMN hosted_by_ndovera INTEGER NOT NULL DEFAULT 0');
        }
        const existing = await readFeatureFlagsRaw();
        if (!existing.length) {
            for (const flag of defaultFlags)
                await writeFeatureFlagRaw(flag.name, flag.enabled);
        }
    })();
    return schemaPromise;
}
async function rebuildLeaderboard(competitionId) {
    await ensureSchema();
    if (pgPool) {
        ...
        try {
            ...
            for (const [index, row] of participants.rows.entries())
                await client.query('INSERT INTO championship_leaderboard (competition_id, user_id, score, time_taken, rank) VALUES ($1, $2, $3, $4, $5)', [competitionId, row.user_id, Number(row.score || 0), Number(row.total_time_taken || 0), index + 1]);
    ...
    if (!sqlite)
        return;
    sqlite.transaction(() => {
        const participants = sqlite.prepare('SELECT competition_id, user_id, score, total_time_taken, joined_at FROM championship_participants WHERE competition_id = ? ORDER BY score DESC, total_time_taken ASC, joined_at ASC').all(competitionId);
...
export async function listCompetitions(params) {
    ...
    const competitionRows = pgPool
        ...
        : (sqlite?.prepare('SELECT c.*, (SELECT COUNT(*) FROM championship_questions q WHERE q.competition_id = c.id) AS question_count, (SELECT COUNT(*) FROM championship_participants p WHERE p.competition_id = c.id) AS participant_count FROM championship_competitions c ORDER BY c.created_at DESC').all() || []);
    const participantRows = params.userId ? (pgPool ? (await pgPool.query('SELECT * FROM championship_participants WHERE user_id = $1', [params.userId])).rows : (sqlite?.prepare('SELECT * FROM championship_participants WHERE user_id = ?').all(params.userId) || [])) : [];
    const leaderboardRows = params.userId ? (pgPool ? (await pgPool.query('SELECT * FROM championship_leaderboard WHERE user_id = $1', [params.userId])).rows : (sqlite?.prepare('SELECT * FROM championship_leaderboard WHERE user_id = ?').all(params.userId) || [])) : [];
    const participantByCompetition = new Map(participantRows.map((row) => [String(row.competition_id), row]));
    const rankByCompetition = new Map(leaderboardRows.map((row) => [String(row.competition_id), Number(row.rank || 0)]));
    return competitionRows.map(rowToCompetition).filter((competition) => {
        if (competition.scope === 'global')
            return includeGlobal;
        if (competition.scope === 'hosted')
            return includeGlobal;
        return !params.schoolId || competition.schoolId === params.schoolId;
    }).map((competition) => {
        const participant = participantByCompetition.get(competition.id);
...
export async function joinCompetition(input) {
    ...
    if (!flags.find((flag) => flag.name === 'championship_enabled')?.enabled)
        throw new Error('Championships are currently disabled.');
    const competitions = await listCompetitions({ schoolId: input.schoolId, userId: input.userId, includeGlobal: true });
    const competition = competitions.find((entry) => entry.id === input.competitionId);
    if (!competition)
        throw new Error('Competition not found.');
    if (competition.scope === 'school' && competition.schoolId && input.schoolId && competition.schoolId !== input.schoolId)
        throw new Error('Competition is not available for your school.');
    if (competition.type === 'exam' && !flags.find((flag) => flag.name === 'hosted_exams_enabled')?.enabled)
        throw new Error('Hosted exams are currently disabled.');
...
export async function submitCompetitionAnswer(input) {
    ...
    if (!participantRow)
        throw new Error('Join the competition before submitting answers.');
    ...
    if (pgPool) {
        ...
        try {
            ...
            const status = answered >= total && total > 0 ? 'submitted' : 'in_progress';
        ...
        sqlite.transaction(() => {
            ...
            const metrics = sqlite.prepare('SELECT COALESCE(SUM(CASE WHEN s.is_correct = 1 THEN q.points ELSE 0 END), 0) AS score, COALESCE(SUM(s.time_taken), 0) AS total_time, COUNT(*) AS answered FROM championship_submissions s JOIN championship_questions q ON q.id = s.question_id WHERE s.participant_id = ?').get(participant.id);
            const total = Number(sqlite.prepare('SELECT COUNT(*) AS total FROM championship_questions WHERE competition_id = ?').get(input.competitionId)?.total || 0);
...
export async function getCompetitionDetail(params) {
    ...
    if (competition.scope === 'school' && competition.schoolId && params.schoolId && competition.schoolId !== params.schoolId)
        throw new Error('Competition is not available for this school.');
    const questionRows = pgPool ? (await pgPool.query('SELECT * FROM championship_questions WHERE competition_id = $1 ORDER BY position ASC', [params.competitionId])).rows : (sqlite?.prepare('SELECT * FROM championship_questions WHERE competition_id = ? ORDER BY position ASC').all(params.competitionId) || []);
    ...
    const submissionRows = participantRow ? (pgPool ? (await pgPool.query('SELECT * FROM championship_submissions WHERE participant_id = $1', [participantRow.id])).rows : (sqlite?.prepare('SELECT * FROM championship_submissions WHERE participant_id = ?').all(participantRow.id) || [])) : [];
    const leaderboardRows = pgPool
        ...
        : (sqlite?.prepare('SELECT l.*, u.name FROM championship_leaderboard l LEFT JOIN users u ON u.id = l.user_id WHERE l.competition_id = ? ORDER BY l.rank ASC LIMIT 10').all(params.competitionId) || []);
    const violationCount = pgPool ? Number((await pgPool.query('SELECT COUNT(*) AS total FROM championship_violations_log v JOIN championship_participants p ON p.id = v.participant_id WHERE p.competition_id = $1', [params.competitionId])).rows[0]?.total || 0) : Number(sqlite?.prepare('SELECT COUNT(*) AS total FROM championship_violations_log v JOIN championship_participants p ON p.id = v.participant_id WHERE p.competition_id = ?').get(params.competitionId)?.total || 0);
    const submissionsByQuestion = new Map(submissionRows.map((row) => { const submission = rowToSubmission(row); return [submission.questionId, submission]; }));
    const questions = questionRows.map(rowToQuestion).map((question) => {
    ...
    return { competition, questions, participant, leaderboard: leaderboardRows.map(rowToLeaderboard), featureFlags, stats: { questionCount: questionRows.length, participantCount: Number(competition.participantCount || 0), submittedCount: participant && participant.status === 'submitted' ? 1 : 0, violationCount } };
}
