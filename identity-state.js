...
function makeSchoolIdFromSubdomain(subdomain) {
...
function ensureStateShape(raw) {
    ...
    if (Array.isArray(raw.schools) && raw.schools.length)
        state.schools = raw.schools;
    if (Array.isArray(raw.users))
        state.users = raw.users;
    if (Array.isArray(raw.students))
        state.students = raw.students;
    if (Array.isArray(raw.transfers))
        state.transfers = raw.transfers;
    if (Array.isArray(raw.lifecycleEvents))
        state.lifecycleEvents = raw.lifecycleEvents;
...
function getSequenceWidth(category) {
...
function getSchoolInitials(schoolName) {
    ...
    if (tokens.length === 0)
        return 'SCH';
    if (tokens.length === 1)
        return tokens[0].slice(0, 3).toUpperCase().padEnd(3, 'X');
    return tokens.slice(0, 4).map((token) => token[0].toUpperCase()).join('');
}
function counterKey(category, scopeId) {
    return `${scopeId}:${category}`;
}
function nextSequence(state, category, scopeId) {
...
export function makeIdentityCode(category, schoolName, sequence) {
    if (category === 'global') {
...
function hashPassword(password) {
    if (!password)
        return bcrypt.hashSync(crypto.randomBytes(8).toString('hex'), 10);
...
function rowToUser(row) {
    return {
        ...
        roles: parseJsonArray(row.roles_json || row.rolesJson),
        activeRole: String(row.active_role || row.activeRole),
        aliases: parseJsonArray(row.aliases_json || row.aliasesJson),
...
function rowToStudent(row) {
    return {
        ...
        name: String(row.name),
        parentUserIds: parseJsonArray(row.parent_user_ids_json || row.parentUserIdsJson),
        status: String(row.status),
        previousUserIds: parseJsonArray(row.previous_user_ids_json || row.previousUserIdsJson),
        createdAt: String(row.created_at || row.createdAt),
        updatedAt: String(row.updated_at || row.updatedAt),
    };
}
...
async function loadIdentityStateInternal() {
    if (pgPool) {
        ...
        return {
            ...
            counters: Object.fromEntries(countersResp.rows.map((row) => [String(row.counter_key), Number(row.value)])),
        };
    }
    if (sqlite) {
        return {
            ...
            counters: Object.fromEntries(sqlite.prepare('SELECT counter_key, value FROM counters').all().map((row) => [row.counter_key, Number(row.value)])),
        };
    }
    return defaultState();
}
async function writeIdentityStateInternal(state) {
    if (pgPool) {
...
async function writeStateWithClient(client, state) {
    ...
    try {
        ...
        for (const school of state.schools)
            await client.query('INSERT INTO schools (id, name, subdomain, created_at) VALUES ($1, $2, $3, $4)', [school.id, school.name, school.subdomain, school.createdAt]);
        ...
        for (const [counterKey, value] of Object.entries(state.counters))
            await client.query('INSERT INTO counters (counter_key, value) VALUES ($1, $2)', [counterKey, value]);
...
export function isIdentityUserActive(user) {
    return (user.status || 'active') === 'active';
}
export function listIdentityUsers(state, schoolId, includeInactive = false) {
...
export function listIdentityLifecycleEvents(state, options = {}) {
...
export function ensureSchool(state, schoolId, schoolName, subdomain) {
    const existing = state.schools.find((school) => school.id === schoolId);
    if (existing) {
        if (!existing.name && schoolName)
            existing.name = schoolName;
        if (!existing.subdomain && subdomain)
            existing.subdomain = subdomain;
        return existing;
    }
    const school = { id: schoolId, name: schoolName, subdomain: subdomain || schoolId, createdAt: nowIso() };
    state.schools.push(school);
    return school;
}
export function countUsersByCategory(state, schoolId) {
...
export function countInactiveUsers(state, schoolId) {
    const scoped = schoolId ? state.users.filter((user) => user.schoolId === schoolId) : state.users;
...
export function findUserByIdentifier(state, identifier) {
...
export function verifyStoredPassword(password, passwordHash) {
    if (!passwordHash)
        return false;
...
export async function provisionUser(state, input) {
    await ensureSchema();
...
export async function createSchoolWithOwner(state, input) {
    await ensureSchema();
...
function defaultRoleForCategory(category) {
    if (category === 'student')
        return 'Student';
    if (category === 'staff')
        return 'Staff';
...
function normalizeRoleForComparison(role) {
...
function createLifecycleEvent(targetUser, input, status) {
    return {
...
export async function assignRoleToUser(state, input) {
    await ensureSchema();
    const targetUser = state.users.find((user) => user.id === input.targetUserId && user.schoolId === input.schoolId);
    if (!targetUser)
        throw new Error('Selected staff member was not found in this school.');
    if (!['staff', 'admin'].includes(targetUser.category))
        throw new Error('Only staff accounts can be appointed to head roles.');
    ...
    if (uniquePerSchool) {
        state.users.forEach((user) => {
            if (user.schoolId !== input.schoolId || user.id === targetUser.id)
                return;
            const nextRoles = user.roles.filter((entry) => normalizeRoleForComparison(entry) !== normalizedRole);
            if (nextRoles.length === user.roles.length)
                return;
...
export async function updateIdentityUser(state, input) {
    await ensureSchema();
    ...
    if (input.phone !== undefined)
        targetUser.phone = normalizePhone(input.phone);
    if (input.name !== undefined)
        targetUser.name = String(input.name || '').trim() || targetUser.name;
    if (input.password !== undefined && String(input.password || '').trim())
        targetUser.passwordHash = hashPassword(String(input.password));
    if (input.category !== undefined)
        targetUser.category = input.category;
    if (input.roles) {
    ...
    if (input.activeRole !== undefined)
        targetUser.activeRole = String(input.activeRole || '').trim() || targetUser.activeRole;
    if (input.aliasToAdd)
        targetUser.aliases = [...new Set([...targetUser.aliases, String(input.aliasToAdd).trim()])];
...
export async function transferStudent(state, studentUserId, targetSchoolId, reason, requestedBy) {
    await ensureSchema();
    ...
    const transfer = { id: crypto.randomUUID(), studentId: studentRecord.id, fromSchoolId, toSchoolId: targetSchool.id, fromUserId, toUserId: nextUserId, reason, requestedBy, createdAt: now, completedAt: now };
