...
export function InputArea({ text, onChange, onPost, onPostMedia, onOpenEmoji }: InputAreaProps) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-30 bg-white dark:bg-slate-950 border-t border-slate-200 dark:border-slate-800 p-3">
      {error && (
        <div className="max-w-3xl mx-auto mb-2 px-2 flex justify-between items-center">
      ...
      <div className="max-w-3xl mx-auto flex items-end gap-1.5">
        <div className="flex items-center gap-0.5 mb-0.5">
          <button
            onClick={onOpenEmoji}
            className="p-2 text-slate-400 hover:text-indigo-500 dark:hover:text-indigo-400 transition-colors rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 flex-shrink-0"
            title="Emojis"
          >
            <Smile size={16} />
          </button>
        ...
        <button
          ...
          disabled={!text.trim()}
          className="p-2 mb-0.5 bg-indigo-500 hover:bg-indigo-600 disabled:bg-slate-200 dark:disabled:bg-slate-800 disabled:text-slate-400 text-white rounded-full transition-all flex-shrink-0 ml-0.5"
        >
