...
export function attachSuperUserFromHeaders(req, res, next) {
...
export function createSuperSessionCookie(user, rememberMe = false) {
    const secret = process.env.NDOVERA_SUPER_ADMIN_AUTH_SECRET?.trim() || process.env.NDOVERA_AUTH_SECRET?.trim();
    if (!secret)
        throw new Error('NDOVERA_SUPER_ADMIN_AUTH_SECRET is not configured.');
...
export function requireSuperRole(...allowed) {
    return (req, res, next) => {
...
export function superHasPermission(user, permission) {
    if (!user)
        return false;
