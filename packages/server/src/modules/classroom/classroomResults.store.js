...
const STATE_PATH = path.resolve(__dirname, '../../../data/classroom-results.json');
...
function inferSectionFromClassName(className) {
...
function getAllowedSectionRoles(classSection) {
...
function gradeFromAverage(total) {
...
function remarkFromAverage(total) {
...
function createDefaultTerm(name) {
  return {
    name,
...
function createDefaultSession(session, feeStatus, outstanding) {
  return {
    session,
...
function ensureStudentRecord(state, user, input) {
...
function sanitizePageTwo(input) {
...
function recomputeTerm(term, updatedSummary) {
...
export function upsertStudentResultForUser(user, input) {
  ...
  if (existingSubjectIndex >= 0) term.subjects[existingSubjectIndex] = nextSubject;
  else term.subjects.push(nextSubject);
  ...
  term.approvals = { ...term.approvals, teacherSavedAt: nowIso() };
...
export function approveStudentResultForUser(user, studentId, sessionName, termName, remark) {
  const state = readState();
  const schoolId = ensureSchoolId(user);
  ...
  if (!session || !term) {
    const error = new Error('Requested session/term result was not found.');
    error.status = 404;
    throw error;
  }
  const role = getUserRole(user);
  ...
  if (role === 'hos') {
    if (!term.approvals.sectionalHeadApprovedAt || !term.summary.sectionalSignature) {
      const error = new Error('The sectional head must approve CA scores before HoS approval.');
      error.status = 400;
      throw error;
    }
    if (term.approvals.hosApprovedAt || term.summary.principalSignature) {
      const error = new Error('The HoS signature has already been applied.');
      error.status = 400;
      throw error;
    }
    ...
    if (term.approvals.sectionalHeadApprovedAt || term.summary.sectionalSignature) {
      const error = new Error('The sectional head signature has already been applied.');
      error.status = 400;
      throw error;
    }
    ...
    const error = new Error('Your current role cannot approve CA scores for this section.');
    error.status = 403;
    throw error;
  }
  record.updatedAt = nowIso();
...
export function listStudentResultsForUser(user, filters) {
  const schoolId = ensureSchoolId(user);
  const role = getUserRole(user);
  const state = readState();
  ...
  if (['teacher', 'school admin', 'hos', 'owner', 'ict', 'ict manager', 'admin', 'super admin', 'tenant school owner', 'head of section', 'sectional head', 'nursery head', 'head teacher', 'junior school principal', 'principal'].includes(role)) {
    return { studentResults, sessions: studentIdFilter ? studentResults[0]?.sessions || [] : [] };
  }
  return { studentResults: [], sessions: studentIdFilter ? studentResults[0]?.sessions || [] : [] };
}