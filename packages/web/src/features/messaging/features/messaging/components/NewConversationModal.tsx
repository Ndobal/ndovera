...
type NewConversationModalProps = {
  users: Record<string, ConversationUser> | ConversationUser[];
  onSelectUser: (users: ConversationUser[], groupName: string) => void;
  onClose: () => void;
};

export default function NewConversationModal({ users, onSelectUser, onClose }: NewConversationModalProps) {
  ...
  const availableUsers = Array.isArray(users) ? users : Object.values(users);
        ...
        <div className="p-4">
          ...
          <div className="max-h-80 overflow-y-auto">
            {availableUsers.filter(user => user.id !== currentUser.id).map(user => (
              ...
              >
                <img src={user.avatarUrl || 'https://placehold.co/80x80?text=U'} alt={user.name} className="w-10 h-10 rounded-full" referrerPolicy="no-referrer"/>
                <div>
