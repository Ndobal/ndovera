  ...
  if (selectedClassworkId) {
    ...
    return (
      ...
      >
        ...
        <div className="glass-panel rounded-xl border border-[#1d4e89]/30 bg-[#0b1f4d] p-5">
          <div className="flex items-start gap-3 mb-4">
            <div className={`p-2.5 rounded-lg shrink-0 ${isDarkMode ? 'bg-[#17336b] text-[#fef3c7] border border-[#facc15]/15' : 'bg-amber-100 text-amber-600'}`}>
              <FileText className="w-5 h-5" />
            </div>
            <div className="flex-1">
              <div className="flex justify-between items-start">
                <h2 className="text-[16px] font-extrabold text-[#facc15] leading-tight">{cw.title}</h2>
                {hasQuestions && <span className="text-sm font-bold text-stone-900 dark:text-[#fef3c7]">{maxScore} pts</span>}
              </div>
              <div className="mt-1 flex items-center gap-1.5 text-[16px] font-bold text-[#facc15]">
          ...
          {cw.description && (
            <div className="whitespace-pre-wrap rounded-lg border border-[#facc15]/10 bg-[#17336b]/45 p-4 text-[16px] font-bold text-[#facc15]">
              {cw.description}
            </div>
          )}
        </div>
        ...
        {isStudent ? (
          <div className="glass-panel rounded-xl p-5">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-[16px] font-extrabold flex items-center gap-2 text-[#facc15]">
                Your Work
              ...
              {mySubmission && mySubmission.totalScore !== undefined && hasQuestions && (
                <div className="text-sm font-bold px-3 py-1 rounded-lg bg-stone-100 dark:bg-[#17336b] dark:text-white">
            ...
            {hasQuestions ? (
              ...
              mySubmission ? (
                <div className="bg-stone-50 dark:bg-[#102a63] border border-stone-200 dark:border-[#1d4e89]/30 rounded-lg p-4">
                  <p className="text-xs text-stone-700 dark:text-white whitespace-pre-wrap">{submissionAnswers['legacy-text'] || mySubmission.content}</p>
                </div>
              ) : (
                <textarea
                  value={answers['legacy-text'] || ''}
            ...
            {mySubmission && (
              <div className="mt-6 pt-4 border-t border-stone-100 dark:border-white/10 text-[10px] text-stone-500 dark:text-stone-300 flex items-center gap-1">
          ...
          <div className="glass-panel rounded-xl p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="text-[10px] font-medium px-2 py-1 rounded-md bg-stone-100 dark:bg-[#17336b] text-stone-600 dark:text-white flex items-center gap-1.5">
                  ...
                  return (
                    <div key={sub.id} className="bg-stone-50 dark:bg-[#102a63] border border-stone-200 dark:border-[#1d4e89]/30 rounded-xl p-4">
                      ...
                      {hasQuestions ? (
                        ...
                        <p className="text-[11px] text-stone-700 dark:text-white whitespace-pre-wrap">{subAnswers['legacy-text'] || sub.content}</p>
                      )}
                    </div>
                  );
                })}
            ...
            {subject.classworks.map((cw) => {
              ...
              return (
                <motion.div
                  ...
                  className="glass-panel rounded-xl bg-[#0b1f4d] p-3 md:p-4 group relative cursor-pointer hover:bg-[#102a63] transition-colors border border-[#1d4e89]/25 hover:border-[#facc15]/20"
                >
                  <div className="flex items-start gap-3">
                    <div className={`p-2 rounded-lg shrink-0 ${isDarkMode ? 'bg-[#17336b] text-[#fef3c7] border border-[#facc15]/15' : 'bg-amber-100 text-amber-600'}`}>
                      <FileText className="w-4 h-4" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 mb-1">
                        <h3 className="text-sm font-bold truncate pr-8 text-[#c2410c] dark:text-orange-400 group-hover:text-[#ea580c] dark:group-hover:text-orange-300 transition-colors">{cw.title}</h3>
                        <div className="flex items-center gap-2">
                          ...
                          <div className="flex items-center gap-1 text-[9px] font-medium text-stone-500 dark:text-[#fde68a] whitespace-nowrap">
                            <Calendar className="w-3 h-3" />
