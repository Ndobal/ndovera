...
import { Save, AlertCircle, FileText, CheckCircle, Mic, Sparkles, Calendar, Users, MapPin } from 'lucide-react';
...
export default function DutyReport() {
  ...
  const [activeTab, setActiveTab] = useState<'my_reports'|'roster'>('my_reports');
  
  const user = loadUser();
  const isHoS = user?.role === 'hos' || user?.role === 'owner' || user?.role === 'HOS';
  ...
  const handleVoiceToText = async (sectionId: string) => {
    try {
      ...
      // Mock Voice to Text
      handleTextChange(sectionId, (formData[sectionId] || '') + ' [Transcribed] All students arrived safely without any major incidents in this area.');
    } catch(e) {
      console.error(e);
    }
  };
  ...
  // Mock roster assignment function
  const assignDuty = (e: React.FormEvent) => {
      e.preventDefault();
      alert("Duty Roster Updated Successfully! (Notifications sent to staff)");
  }
    ...
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      <div className="flex justify-between items-center bg-indigo-50 dark:bg-slate-800 p-4 rounded-xl border border-indigo-100 dark:border-slate-700">
        <h1 className="text-2xl font-bold flex items-center gap-2">
          <FileText className="text-indigo-500" /> Duty Management Status
        </h1>
      ...
      {isHoS && (
        <div className="flex gap-4 border-b border-slate-200 dark:border-slate-700 pb-2">
            <button onClick={() => setActiveTab('roster')} className={\`pb-2 px-1 font-bold \${activeTab === 'roster' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-slate-500'}\`}>Manage Duty Roster</button>
            <button onClick={() => setActiveTab('my_reports')} className={\`pb-2 px-1 font-bold \${activeTab === 'my_reports' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-slate-500'}\`}>Review Submitted Reports</button>
        </div>
      )}

      {isHoS && activeTab === 'roster' && (
          <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-6">
            <div className="mb-4">
                <h2 className="text-lg font-semibold text-slate-900 dark:text-white mb-2">Create / Assign Weekly Duty Roster</h2>
                <p className="text-sm text-slate-500 dark:text-zinc-400">Select teachers and assign them to specific strategic zones.</p>
            </div>
            <form onSubmit={assignDuty} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="p-3 border border-slate-200 dark:border-slate-600 rounded-lg">
                        <label className="text-xs font-bold text-slate-500 mb-1 flex items-center gap-1"><MapPin size={12}/> Zone</label>
                        <select className="w-full bg-slate-50 dark:bg-slate-900 p-2 rounded border-none" required>
                            <option value="">Select Zone...</option>
                            <option>Main Entrance / Gate A</option>
                            <option>Cafeteria / Lunch Hall</option>
                            <option>Playground / Field</option>
                            <option>Library Corridor</option>
                            <option>Bus Park / Transport</option>
                        </select>
                    </div>
                    <div className="p-3 border border-slate-200 dark:border-slate-600 rounded-lg">
                        <label className="text-xs font-bold text-slate-500 mb-1 flex items-center gap-1"><Users size={12}/> Teacher</label>
                        <select className="w-full bg-slate-50 dark:bg-slate-900 p-2 rounded border-none" required>
                            <option value="">Select Teacher...</option>
                            <option>Mr. John Doe</option>
                            <option>Mrs. Clara Smith</option>
                            <option>Ms. Alice M</option>
                        </select>
                    </div>
                    <div className="p-3 border border-slate-200 dark:border-slate-600 rounded-lg">
                        <label className="text-xs font-bold text-slate-500 mb-1 flex items-center gap-1"><Calendar size={12}/> Days Active</label>
                        <input type="text" placeholder="e.g. Mon-Wed" className="w-full bg-slate-50 dark:bg-slate-900 p-2 rounded border-none" required />
                    </div>
                </div>
                <button type="submit" className="px-6 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg shadow">Publish to Roster</button>
            </form>

            <div className="mt-8 border-t border-slate-200 dark:border-slate-700 pt-6">
                <h3 className="text-sm font-bold text-slate-700 dark:text-slate-300 mb-4">Current Active Roster</h3>
                <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700 border-dashed">
                    <p className="text-xs text-slate-500">Main Entrance: <strong>Mr. John Doe</strong> (Mon-Fri) <br/> Playground: <strong>Mrs. Clara</strong> (Tue-Thu)</p>
                </div>
            </div>
          </div>
      )}

      {(!isHoS || (isHoS && activeTab === 'my_reports')) && (
...
console.log('DutyReport patched with HoS Assignment Roster');
