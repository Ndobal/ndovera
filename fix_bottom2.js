...
const badPatternStart = "              const srv = \n\n// ==================== AURAS & PREMIUM FEATURES ====================";

const aurasBlock = `// ==================== AURAS & PREMIUM FEATURES ====================
...
app.post('/api/auras/deduct', requireAuth, (req, res) => {
  ...
  const user = db.prepare("SELECT auras FROM users WHERE id = ?").get(req.user.id);
  if (user.auras < amount) return res.status(403).json({ error: 'Insufficient AURAS. Please top up.' });
...
app.get('/api/duty-report', requireAuth, (req, res) => {
  try {
    ...
    if (!isHoS) reports = reports.filter(r => r.staff_id === req.user.id);
    res.json({ reports });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
let targetIdx = code.indexOf(badPatternStart);
if (targetIdx !== -1) {
  const listenIdx = code.indexOf("app.listen(p, '0.0.0.0', () => {", targetIdx);
  const strippedCode = code.substring(0, targetIdx) + "              const srv = \n" + code.substring(listenIdx);
  const tryListenIdx = strippedCode.indexOf('  async function tryListen(basePort: number, attempts = 10) {');
  const finalCode = strippedCode.substring(0, tryListenIdx) + aurasBlock + '\n\n' + strippedCode.substring(tryListenIdx);
  fs.writeFileSync('packages/server/server.ts', finalCode);
